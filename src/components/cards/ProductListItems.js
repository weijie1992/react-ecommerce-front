...
const ProductListItems = ({product}) => {
    const {title, images, price,category,subs,shipping,color,brand,quantity,sold} = product;
    return (
        ...
        <ul className="list-group">
            ...
            {subs && (<li className="list-group-item">
                Sub Categories
                {subs.map((sub)=> {
                     return (<Link to={`/sub/${sub.slug}`} key={sub._id} className="label label-default label-pill pull-xs-right">{sub.name}</Link>)
                })}
            </li>)}
